	...
	protected boolean doMatch(Map<String, Object> param,
		...
		if(count>1){
			...
			if(latestSuccessTimes>=(1+percentIncrease)*averageSuccessTimesForCompare){
				param.put(AlertParamKey.ALERT_MSG, "The success times is "+percentIncrease+" times bigger than the average data in pervious "+minutesToCompare+" minute(s). App:["+day.getAppName()+"], service:["+day.getService()+"], method:["+day.getMethod()+"], current time:["+DateUtil.changeHHmm(day.getTime(), latestHhmm)  +"], current successTimes:["+day.getSuccessTimes()+"]");
				return true;
			}
		}
